...
def fb_row():
    return {
        ...
        "category": "general", "message": "Test feedback",
        "status": "pending", "created_at": "2026-01-01T10:00:00"
    }


def _find_valid_category():
    """Probe which category enum the backend accepts. Runs once at import time."""
    for cat in ["general", "content", "technical", "bug", "feature", "other", "suggestion"]:
        mock_sb.reset_mock()
        # Setup user fetch
        mock_sb.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value = MagicMock(data=[make_user("teacher")])
...
VALID_CAT = _find_valid_category()
...
class TestSubmitFeedback:
    ...
    def test_teacher_200(self):
        ...
        mock_sb.table.return_value.insert.return_value.execute.return_value = MagicMock(data=[fb_row()])
        r = client.post("/api/feedback/",
                        json={"category": VALID_CAT, "message": "Good"},
                        headers=auth("teacher"))
    ...
    def test_student_blocked_403(self):
        mock_user("student")
        r = client.post("/api/feedback/",
                        json={"category": VALID_CAT, "message": "Hi"},
    ...
    def test_never_500(self):
        ...
        mock_sb.table.return_value.insert.return_value.execute.return_value = MagicMock(data=[fb_row()])
        r = client.post("/api/feedback/",
                        json={"category": VALID_CAT, "message": "Fine"},
                        headers=auth("teacher"))
        assert r.status_code != 500
...
class TestGetFeedback:
    ...
    def test_admin_200(self):
        mock_user("admin")
        # feedback.py GET: .select("*, users!sender_id(...)").order(...).execute()
...
# ── My Feedback ───────────────────────────────────────────────────────────────
# feedback.py /mine: .select("*").eq("sender_id", id).order(...).execute()
# get_current_user: .select("*").eq("id", user_id).limit(1).execute()
# Both use eq — set BOTH chains

class TestMyFeedback:

    def test_teacher_gets_list(self):
        mock_user("teacher")
        # /mine chain: select().eq().order().execute()
...
# ── Student Feedback ──────────────────────────────────────────────────────────
# student_feedback.py POST/GET at "" (no trailing slash)
# prefix in main.py: /api/student-feedback
...
class TestStudentFeedback:
    ...
    def test_student_post_200(self):
        ...
        r = client.post("/api/student-feedback",
    ...
    def test_admin_can_view_200(self):
        mock_user("admin")
        # student_feedback GET: .select("*").order(...).execute()
        mock_sb.table.return_value.select.return_value.order.return_value.execute.return_value = MagicMock(data=[
