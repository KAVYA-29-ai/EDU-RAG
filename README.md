...
<p align="center">
  ...
  <img src="https://img.shields.io/github/license/KAVYA-29-ai/EDU-RAG?style=for-the-badge" alt="License" />
</p>

<p align="center">
  <img src="https://github.com/KAVYA-29-ai/EDU-RAG/actions/workflows/ci.yml/badge.svg" alt="CI Status" />
</p>
...
1. **User Authentication** — Users register/login with institution ID and password. JWT tokens are issued for secure access.
2. **PDF Upload & Indexing** — Teachers/Admins upload PDFs. Text and images are extracted, chunked, and embedded using Gemini AI. All data is stored in Supabase.
3. **RAG Search** — Students/Teachers/Admins can ask questions. The system finds relevant chunks using vector similarity and generates an answer with source citations using Gemini.
4. **Feedback & Analytics** — Teachers submit feedback to admins. Analytics dashboards show usage, trending topics, and engagement.

See [docs/SETUP.md](docs/SETUP.md) for local setup and [docs/API.md](docs/API.md) for API reference.
...
- `routers/` — HTTP transport and request/response mapping
- `core/` — cross-cutting framework concerns (RBAC dependencies)
- `services/` — reusable business logic (chat operations, websocket manager)
- `models.py` — schema validation contracts
...
| Control | Status | File |
|---------|--------|------|
| Password Hashing | bcrypt via passlib | `backend/routers/auth.py` |
| Authentication | JWT Bearer tokens | `backend/routers/auth.py` |
| Authorization/RBAC | Role guards | `backend/core/rbac.py` |
| Rate Limiting | SlowAPI + middleware | `backend/core/rate_limit.py` |
| CORS | CORSMiddleware | `backend/main.py` |
| Security Headers | Custom middleware | `backend/main.py` |
| Input Validation | bleach + validators | `backend/models.py` |
| SQL Injection | ORM (SQLAlchemy) parameterized queries | `backend/database.py` |
| XSS Prevention | bleach sanitization | `backend/models.py` |
| Trusted Hosts | TrustedHostMiddleware | `backend/main.py` |
...
2. **Import in Vercel** — Go to [vercel.com/new](https://vercel.com/new) and import `KAVYA-29-ai/EDU-RAG`
...
```bash
# Clone
git clone https://github.com/KAVYA-29-ai/EDU-RAG.git
cd EDU-RAG
...
| `feedback` | Teacher to Admin feedback with responses |
| `student_feedback` | Student to Teacher anonymous feedback |
...
FastAPI auto-generated docs available at `/docs` (Swagger UI) and `/redoc` (ReDoc).
...
| POST | `/api/rag/pdfs/{id}/index` | Index PDF for search |
...
- [x] Rate limiting (SlowAPI + in-memory middleware)
- [x] Security headers middleware (HSTS, CSP, XFO)
- [x] Input sanitization (bleach + Pydantic validators)
- [x] CI pipeline (Backend Tests + Frontend Tests + Lint)
...
**Team Leader** — Khushi Saraswat — [github.com/khushisara1](https://github.com/khushisara1)

**Contributors**

- Kavya Rajput — [github.com/KAVYA-29-ai](https://github.com/KAVYA-29-ai)
- Harshita Shakya — [github.com/HarshitaShakya](https://github.com/HarshitaShakya)

**Repository** — [github.com/KAVYA-29-ai/EDU-RAG](https://github.com/KAVYA-29-ai/EDU-RAG)

---

[Report Bug](https://github.com/KAVYA-29-ai/EDU-RAG/issues) · [Request Feature](https://github.com/KAVYA-29-ai/EDU-RAG/issues)
